...
async def start(message: types.Message):
    ...
    # Попробуйте, за исключением также может быть использовано здесь. Это предотвратит ошибки!
    try:
...
async def show_statistics(message: types.Message):
    ...
    project_data = dict(cursor.fetchone()) if cursor.fetchone() else {}
...
@dp.callback_query_handler(text="change_support_link")
async def change_name(callback_data: types.CallbackQuery):
    await callback_data.message.edit_text(f"⬜ {hbold('Укажите новую ссылку на саппорта')}:", parse_mode=DEFAULT_PARSE_MODE)

    await States.change_support_link.set()
...
async def back(callback_data: types.CallbackQuery):
    cursor.execute(f"SELECT * FROM `project` WHERE creator_id = {callback_data.from_user.id};")
    project = dict(cursor.fetchone())
    text = f"💻 {hbold('Настройки')}:\n\n⬜ {hbold('Название проекта')}: " \
           f"{project['project_name'] if project['project_name'] is not None else 'Не установлено'}\n⬜ " \
           f"{hbold('Ссылка на чат')}: {project['chat_link'] if project['chat_link'] is not None else 'Не установлено'}\n⬜ " \
